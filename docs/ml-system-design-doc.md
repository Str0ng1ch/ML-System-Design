...
#### Инфраструктура, в которой будет работать решение:

1. Yandex Cloud как основная платформа + интеграция с Yandex LLM API;
2. Backend развернут на виртуальной машине Yandex в виде docker контейнера;
3. Телеграм-бот как поверхность для взаимодействия с пользователем.

#### Требования к быстродействию, надежности и масштабированию системы:

1. SLA 99% в активные часы работы (предположительно 9:00-23:00);
2. Время ответа до 1 минуты для 95% запросов;
3. Для надежности автоматический рестарт контейнера в случае сбоев, сохранение логов, оценок пользователей и работ в
   примонтированную директору, чтобы избежать потерю данных при рестарте контейнера;
4. Для безопасности системы введены следующие меры: защита от prompt-инъекций, анонимизация работ студентов, ограничение
   поддерживаемых форматов и сканирование файлов, 1 попытка в день для генерации рекомендаций.

#### Нагрузочное тестирование:

1. Проведено пилотное тестирование на группе студентов, в результате которого бот активно использовался в течение дня.
   Сбоев и downtime не обнаружено. Время ответа не возросло более чем на 55 секунд.
2. Проведено отдельно одновременное тестирование генерации ответа от LLM на 4 аккаунтах. Время ответа в пределах нормы.
3. Почему НТ слабо можно применить к этой системе? Количество студентов, для которых разработана система не превышает
   50 человек из нескольких студенческих групп. Группы получают задание в разное время, из-за чего ожидается низкое
   количество одновременных запросов (при условии сохранения лимита в 1 рекомендацию в день, введенную чтобы избежать
   злоупотреблением бота)
